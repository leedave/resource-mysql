...
class Update extends Mysql
{
    ...
    {
        $arrFile = explode(DIRECTORY_SEPARATOR, (string) $filePath);
        $fileName = array_pop($arrFile);
        $folder = implode(DIRECTORY_SEPARATOR, $arrFile).DIRECTORY_SEPARATOR;

        $folder = str_replace(static::getDocRoot(), "", (string) realpath($folder));

        $tmp = new Update();
        $tmp->name = $fileName;
        $tmp->folder = $folder."/";
        $tmp->createDate = date("Y-m-d H:i:s");
        $tmp->save();
    ...
    protected function cleanFolderIfInOldFormat(array $row)
    {
        ...
        if (isset($row['folder']) && strpos($row['folder'], $docroot) === 0) {
            $entry = new Update();
            $entry->load($row['id']);
            $entry->folder = str_replace($docroot, "", (string) realpath($row['folder']))."/";
            $entry->save();
        }
    }
    ...
    protected function findNewFiles(string $path): array
    {
        ...
        foreach($arrReturn as $key => $file) {
            $filePathShort = str_replace(static::getDocRoot(), "", (string) realpath($file));
